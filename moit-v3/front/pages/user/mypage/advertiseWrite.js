...
function AdvertiseWritePage() {
        ...
        const fetchPrices = async () => {
            try {
                ...
                const positionRes = await axios.get(
                    "/api/advertisement/prices/position",
                );
                ...
                const formattedPositionPrices = positionRes.data.map(
                    (item) => ({
                        key: item.position,
    ...
    const handleAiGenerate = async () => {
        ...
        try {
            ...
            form.setFieldsValue({
                ...
                targetAgeMin: data.targetAgeMin ?? "",
                targetAgeMax: data.targetAgeMax ?? "",
                targetGender: ["ALL", "MALE", "FEMALE"].includes(
                    data.targetGender,
                )
                    ? data.targetGender
                    : "ALL",
    ...
    const calculateAdPrice = (days, grade) => {
        ...
        // 백엔드와 동일
        // ==========================================
        if (remainingDays > 0 && prices.length > 0) {
            const smallestPrice = prices[prices.length - 1];

            const smallestPeriod = smallestPrice.days;

            if (smallestPeriod > 0) {
                const count = Math.ceil(remainingDays / smallestPeriod);
    ...
    const handleImageChange = (type, { fileList }) => {
        ...
        if (existingImageTypes.includes(type) && newFileList.length === 0) {
    ...
    const handleSubmit = async (values) => {
        try {
            ...
            const hasMainImage =
                mainFiles &&
                mainFiles.length > 0 &&
                (mainFiles[0].originFileObj || mainFiles[0].url);

            if (!hasMainImage) {
                message.error(
                    "메인 이미지는 필수입니다. 이미지를 등록해주세요.",
                );
                setLoading(false);
                return;
            }
            ...
            deletedImageTypes.forEach((type) => {
                formData.append("deletedImageTypes", type);
            });
    ...
    return (
        <div style={{ padding: 24 }}>
            ...
            >
                ...
                <Button
                    onClick={() => router.push("/user/mypage/advertiseList")}
                >
                    목록
                </Button>
            </div>
            ...
            <Card>
                <Form
                    ...
                    initialValues={{ targetGender: "ALL", adGrade: "GENERAL" }}
                >
                    {/* 기본 정보 */}
                    <h3>기본 정보</h3>
                    <Form.Item
                            ...
                            <tbody>
                                {adPrices.map((price) => (
                                    <tr key={price.days}>
                                        ...
                                        >
                                            {price.generalPrice.toLocaleString()}
                                            원
                                        </td>
                                        <td
                                            style={{
                    ...
                    {imageConfigs.map((config) => (
                        ...
                        >
                            ...
                            <Upload
                                listType="picture"
                                fileList={imageFiles[config.type]}
                                beforeUpload={() => false}
                                onChange={(info) =>
                                    handleImageChange(config.type, info)
                                }
                                maxCount={1}
                                accept="image/*"
                            >
                                {imageFiles[config.type].length === 0 && (
                    ...
                    <Form.Item shouldUpdate>
                        {({ getFieldValue }) => {
                            ...
                            const activePositions = Object.keys(
                                imageFiles,
                            ).filter(
                                (key) =>
                                    imageFiles[key] &&
                            ...
                            const positionExtra = activePositions.reduce(
                                (sum, position) => {
                                    const item = positionPrices.find(
                                        (p) => p.key === position,
                                    );
